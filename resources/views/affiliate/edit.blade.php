<x-layout.affiliate>
    <div class="w-full h-screen flex flex-col items-center my-10">
        ...
        <form class="max-w-[600px]" method="POST" action="{{ route('affiliate.update', $sortition->id) }}">
            ...
            <input type="hidden" name="user_id" id="user_id" value="{{ old('user_id', $sortition->user_id) }}">

            <label class="text-sm" for="title">Título</label>
            <input
                type="text" name="title" id="title" value="{{ old('title', $sortition->title) }}" placeholder="Título do sorteio"
            ...
            <div class="flex gap-2 my-4">
                <div class="w-1/2">
                    ...
                    <input
                        name="price" id="price" type="number" step="0.01" value="{{ old('price', $sortition->price) }}" placeholder="Ex: 10.00"
                        class="w-full p-2 mb-3 text-white border border-[#363333] rounded"
                    >
                </div>
                <div class="w-1/2">
